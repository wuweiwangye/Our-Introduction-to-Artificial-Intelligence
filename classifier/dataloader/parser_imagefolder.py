...
from classifier.dataloader import data_resample
...
class ImageFolderDataset(Dataset):
    ...
    def __init__(self, image_dir_list, class_name=None, transform=None,
        ...
        self.class_dict, self.item_list = self.get_item_list_classes(image_dir_list, self.class_dict, shuffle,
                                                                     only_id=True)
    ...
    @staticmethod
    def get_item_list_classes(image_dir_list, class_dict=None, shuffle=True, only_id=True):
        """
        ...
        for i, image_dir in enumerate(image_dir_list):
            ...
            if not os.path.exists(image_dir):
                raise Exception("image_dir:{}".format(image_dir))
            image_list, label_list = file_utils.get_files_labels(image_dir, postfix=IMG_POSTFIX)
            if not only_id:
        ...
        item_list = ImageFolderDataset.get_item_list(image_lists, image_labels, class_dict)
        if shuffle:
            random.seed(100)
...
if __name__ == '__main__':
    image_dir1 = "PyTorch-Classification-Trainer/data/face-mask/train"
    input_size = [128, 128]
    ...
    batch_size = 1
    dataset_train = ImageFolderDataset(image_dir_list=image_dir1,
                                       transform=transform,
                                       resample=False,
                                       shuffle=False,
                                       disp=True)
